...
import { tradingLogger, debugLogger, websocketLogger, closeLoggers } from './lib/base-logger';
...
config.accounts.forEach((account) => {
    const accountManager = new AccountManager(`account-data-${account.name}.json`);
    accountManager.initializeAccountData(account.name, config.pairs);

    // Create trade manager for each account and store in Map
    tradeManagers.set(account.name, new TradeManager(
        account.name,
        accountManager,
        account.api_key,
        account.api_secret
